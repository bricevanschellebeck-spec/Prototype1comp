...
export function RepresentationReview({ spec, plan, modelId, onCompile, onReplan }: { spec: ApprovedLearningSpec; plan: RepresentationPlanDraft; modelId: string; onCompile: (ids: string[]) => Promise<void>; onReplan: () => void }) {
  ...
  return <section className={`${styles.reviewLab} ${styles.plannerReview} ${styles.stageSurface}`}>
    ...
    <footer className={styles.reviewFooter}><div><button disabled={working} type="button" onClick={onReplan}>Plan again</button>{error ? <span>{error}</span> : <small>No generated HTML, CSS, coordinates, or invented data.</small>}</div><button disabled={working} type="button" onClick={compile}>{working ? "Validating and compiling…" : "Compile approved blocks →"}</button></footer>
  </section>;
}
