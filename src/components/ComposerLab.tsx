...
export function ComposerLab() {
  ...
  function buildRequest(selectedTimeChoice = timeChoice): CompositionRequest {
    ...
    const prototypeMinutes = selectedTimeChoice === 5 ? 5 : 9;

    return {
      topicId: OHMS_LAW_TOPIC_ID,
      requiredObjectiveIds: [RESISTANCE_OBJECTIVE_ID],
      learnerSnapshot: {
      ...
      constraints: {
        ...
        maximumSteps: selectedTimeChoice === 5 ? 3 : 5,
        unavailableBlockIds: [],
        accessibilityNeeds: [],
      },
    };
  ...
  async function buildLearningPath(selectedTimeChoice = timeChoice) {
    ...
    try {
      const response = await fetch("/api/compose", {
        ...
        body: JSON.stringify(buildRequest(selectedTimeChoice)),
      });
      const payload = (await response.json()) as ComposeResponse | { error: string };
      if (!response.ok || "error" in payload) {
        throw new Error("error" in payload ? payload.error : "Composition failed.");
      }
      setResult(payload);
    } catch (caughtError) {
      setError(
