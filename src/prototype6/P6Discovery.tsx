...
type FieldId = "space" | "electricity" | "mathematics" | "biology" | "history" | "engineering" | "computing" | "physics";

const discoverySectionIds = ["p6-knowledge-eye", "p6-curiosity-trails", "p6-academic-map"];
...
export function P6Brand() {
  return <div className={styles.brand}><span>✦</span><strong>Curiosity Lab</strong><small>Prototype 6</small></div>;
}

export function P6DiscoveryBrand() {
  return (
    <div className="brand-mark" aria-label="Curiosity Lab Prototype 6">
...
export function P6Discovery({ onContinue }: { onContinue: () => void }) {
  ...
  function previewSubject(exampleQuestion: string) {
    ...
    setQuestion(exampleQuestion);
    scrollToSection("p6-knowledge-eye");
  }

  return (
    <div ref={pageRef} className={`curiosity-page stage-enter ${styles.originalDiscovery}${isLaunching ? " launching" : ""}`}>
      <header className="site-header">
        <P6DiscoveryBrand />
        <div className="header-note"><span className="live-dot" aria-hidden="true" />Autonomous lesson assembly · trusted circuit knowledge</div>
      </header>

      <main className="field-main">
        <section id="p6-knowledge-eye" className="curiosity-field" aria-labelledby="p6-curiosity-heading">
          ...
          <div className="curiosity-core">
            ...
            <h1 id="p6-curiosity-heading">What are you curious about?</h1>
            <p>Ask a question. Trusted knowledge will be assembled into an experience for you.</p>
            <form className="curiosity-search" onSubmit={submit}>
              <SearchIcon />
              <label className="sr-only" htmlFor="p6-curiosity-question">What do you want to understand?</label>
              <input id="p6-curiosity-question" type="text" value={question} onChange={(event) => { setQuestion(event.target.value); setSelectedField(null); }} placeholder="Ask about anything…" autoComplete="off" />
              <button type="submit" aria-label="Continue with this question"><span>Explore</span><span aria-hidden="true">→</span></button>
            </form>
            <span className="prototype-hint"><i aria-hidden="true" /> Prototype 6 demonstrates resistance and current</span>
          </div>
          ...
          <div className="sector-menu" aria-label="Knowledge fields">
            ...
            <button className="knowledge-fragment fragment-circuit live-fragment" type="button" onClick={launch}>
              <div className="sector-content"><svg viewBox="0 0 150 78" aria-hidden="true"><path className="mini-wire" d="M12 22h40m46 0h39v42H12V46" /><path className="mini-resistor" d="m52 22 8-10 10 20 10-20 10 20 8-10" /><path className="mini-current" d="M38 64h54" /><path className="mini-arrow" d="m91 64-10-7v14Z" /><circle className="mini-bulb" cx="137" cy="64" r="9" /><path className="mini-battery" d="M5 33h14M8 42h8" /></svg><span>Electricity</span><strong>How does resistance change current?</strong><small>Build the live lesson →</small></div>
            </button>
          ...
          <button className="scroll-cue" type="button" onClick={() => scrollToSection("p6-curiosity-trails")}><span>More ways in</span><i aria-hidden="true">↓</i></button>
        </section>

        <section id="p6-curiosity-trails" className="topic-screen curiosity-trails">
          <div className="topic-screen-shell">
            ...
            <button className="next-screen-button" type="button" onClick={() => scrollToSection("p6-academic-map")}>See academic fields <span aria-hidden="true">↓</span></button>
          </div>
        </section>

        <section id="p6-academic-map" className="topic-screen academic-map">
          <div className="topic-screen-shell">
            <header className="topic-screen-heading compact-topic-heading"><div><span className="screen-index">03</span><span className="eyebrow">Academic foundations</span></div><h2>A broader map of what you could understand.</h2><p>Prototype 6 autonomously assembles the circuit demonstration from one approved knowledge package. Choose a question, then continue from the eye.</p></header>
            ...
            <button className="next-screen-button return-eye-button" type="button" onClick={() => scrollToSection("p6-knowledge-eye")}>Return to the knowledge eye <span aria-hidden="true">↑</span></button>
          </div>
        </section>
      </main>
    </div>
