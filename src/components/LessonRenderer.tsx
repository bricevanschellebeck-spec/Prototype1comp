...
import { CircuitFoundationBlock } from "@/src/components/blocks/CircuitFoundationBlock";
...
import { VoltageDiscoveryBlock } from "@/src/components/blocks/VoltageDiscoveryBlock";
...
function renderBlock(
  block: LearningBlock,
  callbacks: {
    onSimulationInteraction: (blockId: string) => void;
    onPredictionResult: (correct: boolean) => void;
    onChallengeComplete: (blockId: string) => void;
  },
) {
  if (block.componentType === "diagram") return <CircuitDiagramBlock block={block} />;
  if (block.content.kind === "circuit-foundation") {
    return (
      <CircuitFoundationBlock
        block={block}
        onInteraction={() => callbacks.onSimulationInteraction(block.id)}
      />
    );
  }
  if (block.content.kind === "voltage-discovery") {
    return (
      <VoltageDiscoveryBlock
        block={block}
        onInteraction={() => callbacks.onSimulationInteraction(block.id)}
      />
    );
  ...
  if (block.componentType === "challenge") {
    return (
      <ChallengeBlock
        block={block}
        onComplete={() => callbacks.onChallengeComplete(block.id)}
      />
    );
  }
  return null;
...
function learningNote(block: LearningBlock) {
  ...
  if (block.content.kind === "circuit-foundation") {
    return "Current flows only when the components form one complete, closed path.";
  }
  if (block.content.kind === "voltage-discovery") {
    return "A battery provides the electrical push called voltage.";
  }
  ...
  return "Current equals voltage divided by resistance: I = V ÷ R.";
}
...
export function LessonRenderer({
  ...
  const [exploredSimulationIds, setExploredSimulationIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const waitingForExperiment =
    active?.block.componentType === "simulation" &&
    !exploredSimulationIds.has(active.block.id);
  const waitingForPrediction =
    active?.block.componentType === "question" && checkpointResult === null;
  const waitingForChallenge =
    active?.block.componentType === "challenge" &&
    !completedChallengeIds.has(active.block.id);
  const continueBlocked = waitingForExperiment || waitingForPrediction || waitingForChallenge;
  ...
  function nextActionLabel() {
    if (waitingForExperiment && active?.block.content.kind === "circuit-foundation") {
      return "Close the circuit to continue";
    }
    if (waitingForExperiment && active?.block.content.kind === "voltage-discovery") {
      return "Remove the battery to compare";
    }
    if (waitingForExperiment) return "Predict, then change resistance";
    if (waitingForPrediction) return "Answer to reveal the next block";
    if (waitingForChallenge) return "Connect the formula, then apply it";
          ...
          {active ? (
            <div className={`exhibit-scene${active.adaptive ? " adaptive-exhibit" : ""}`} key={active.step.stepId}>
              ...
              {renderBlock(active.block, {
                onSimulationInteraction: (blockId) => {
                  setExploredSimulationIds((current) => {
